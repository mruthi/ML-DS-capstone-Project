...
class dataUploadView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            data_chi=request.POST.get('children')
            data_sex=request.POST.get('sex_male')
            data_smo=request.POST.get('smoker_yes')
            #print (data)
            #dataset1=pd.read_csv("prep.csv",index_col=None)
            dicc={'yes':1,'no':0}
            filename = 'finalized_model_RF_ASSIGN.sav'
            classifier = pickle.load(open(filename, 'rb'))

            data = np.array([data_age,data_bmi,data_chi,data_sex,data_smo])
            #sc = StandardScaler()
...
#data_for_prediction =obj.pca(np.array(data_for_prediction),y_test)
            ...
            return render(request, "succ_msg.html", {'data_age':data_age,'data_bmi':data_bmi,'data_chi':data_chi,'data_sex':data_sex,'data_smo':data_smo,
                                                        'out':out})
